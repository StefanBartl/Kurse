...
void BWString::_append_split_array(const BWString & s) const {
    ...
    if (!_split_count) {
        _split_array.reset(new _bwsp[__BWString__MAX_SPLIT + 1]);
    }
...
const BWString::split_ptr & BWString::split( const char * match, int max_split ) const {
    ...
    if (max_split < 0) max_split = __BWString__MAX_SPLIT;
    
    size_t match_len = strnlen(match, __BWString__MAX_LEN);
    if(match_len >= __BWString__MAX_LEN) return _split_array;
